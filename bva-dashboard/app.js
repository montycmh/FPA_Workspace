...
(function(){
  ...
  function saveStateQuietly(){
    if(!state.model) return;
    const key = storageKey();
    const payload = { comments: {}, actions: {}, hidden: [] };
    document.querySelectorAll('[data-persist]').forEach(el => payload.comments[el.dataset.persist] = el.value);
    document.querySelectorAll('[data-act-cb]').forEach(cb => payload.actions[cb.dataset.actCb] = cb.checked);
    document.querySelectorAll('.hidden[id]').forEach(el => payload.hidden.push(el.id));
    localStorage.setItem(key, JSON.stringify(payload));
  ...
  function downloadHtml(){
    ...
    Array.from(document.styleSheets || []).forEach(ss => {
      try{
        const href = ss.href || '';
        if(href && !href.startsWith(window.location.origin) && !href.startsWith('file:')) return;
        inlineCss += Array.from(ss.cssRules || []).map(r => r.cssText).join('
') + '
';
      }catch(e){}
    });
...
' + clone.outerHTML;
    const filename = `bva_dashboard_${slug(state.model.meta.dashboardCode)}_${slug(state.model.meta.fyToken)}_${slug(state.model.meta.monthToken)}.html`;
