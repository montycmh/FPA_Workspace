...
(function(){
  ...
  function renderDashboard(model){
    ...
    const html = `
      ...
      ${renderVarianceSection('sec-qplan', `${model.meta.currentQuarterLabel} vs ${model.meta.fyToken} Plan`, model.quarter.topPlan, model.quarter.driverBlocksPlan, model.quarter.l2Rows, model.quarter.expense, 'p', model.quarter.monthLabels, model.quarter.expense.total.w, model.quarter.expense.total.p)}
      ${renderVarianceSection('sec-qfcst', `${model.meta.currentQuarterLabel} vs ${model.meta.forecastLabel}`, model.quarter.topFcst, model.quarter.driverBlocksFcst, model.quarter.l2Rows, model.quarter.expense, 'f', model.quarter.monthLabels, model.quarter.expense.total.w, model.quarter.expense.total.f)}
      ${renderYearSection('sec-fyplan', `Full Year vs ${model.meta.fyToken} Plan`, model.year.topPlan, model.year.driverBlocksPlan, model.year.l2Rows, model.year.expense, 'p', model.year.expense.total.w, model.year.expense.total.p)}
      ${renderYearSection('sec-fyfcst', `Full Year vs ${model.meta.forecastLabel}`, model.year.topFcst, model.year.driverBlocksFcst, model.year.l2Rows, model.year.expense, 'f', model.year.expense.total.w, model.year.expense.total.f)}
      ${renderTE(model)}
      ${renderHC(model)}
      ${renderActions(model)}
    `;
  ...
  function renderVarianceSection(id, title, topBundle, blocks, rows, expenseRow, benchmarkKey, monthLabels, workingEnd, baseStart){
    ...
    return `<section class="sec" id="${id}">
      ...
      <div class="tbl-wrap">${renderQuarterTable(rows, expenseRow, benchmarkKey, monthLabels)}</div>
  ...
  function renderYearSection(id, title, topBundle, blocks, rows, expenseRow, benchmarkKey, workingEnd, baseStart){
    const labels = ['Benchmark', ...topBundle.rows.map(r => cleanLabel(r.label)), 'Others', 'Working'];
    const vars = [...topBundle.rows.map(r=>r.variance), topBundle.others];
    return `<section class="sec" id="${id}">
      ...
      <div class="wf-wrap"><canvas id="${id}-wf"></canvas></div>
      <div class="sublbl">L2 detail</div>
      <div class="tbl-outer"><div class="tbl-inner">${renderYearTable(rows, expenseRow, benchmarkKey)}</div></div>
      <div class="sublbl">Drivers</div>
      ${blocks.map(b => renderDriverBlock(b)).join('') || '<div class="comment-block">No driver block crossed the materiality threshold for this section.</div>'}
      <div class="sublbl">Additional Comments</div>
      <div class="comment-block" data-comment-block="${id}-comments">${renderAdditionalComments(`${id}-comments`)}</div>
  ...
  function renderQuarterTable(rows, expenseRow, benchmarkKey, monthLabels){
    ...
    h += '</tr></thead><tbody>';
    const totalRow = expenseRow ? [{ ...expenseRow, label: expenseRow.label || 'Expense', rowType: 'expense' }] : [];
    const orderedRows = rows.filter(r => r.rowType !== 'expense').concat(totalRow);
  ...
  function renderYearTable(rows, expenseRow, benchmarkKey){
