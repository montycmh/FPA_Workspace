...
(function(){
  ...
  document.getElementById('pdf-nav').addEventListener('click', printBoard);
...
function syncFormStateIntoClone(cloneDoc){
    document.querySelectorAll('textarea').forEach((el, i) => {
      const target = cloneDoc.querySelectorAll('textarea')[i];
      if(target) target.textContent = el.value;
    });
    document.querySelectorAll('input[type="checkbox"]').forEach((el, i) => {
      const target = cloneDoc.querySelectorAll('input[type="checkbox"]')[i];
      if(target){
        target.checked = el.checked;
        if(el.checked) target.setAttribute('checked', 'checked'); else target.removeAttribute('checked');
      }
    });
    document.querySelectorAll('input:not([type="checkbox"]):not([type="file"])').forEach((el, i) => {
      const target = cloneDoc.querySelectorAll('input:not([type="checkbox"]):not([type="file"])')[i];
      if(target) target.setAttribute('value', el.value || '');
    });
  }

  function replaceCanvasWithImages(cloneDoc){
    const origCanvases = Array.from(document.querySelectorAll('canvas'));
    const cloneCanvases = Array.from(cloneDoc.querySelectorAll('canvas'));
    origCanvases.forEach((canvas, i) => {
      ...
      try{
        ...
        img.setAttribute('data-export-from', target.id || 'canvas');
        target.replaceWith(img);
      }catch(err){ console.warn('Could not serialize canvas', err); }
    });
  }

  async function buildStandaloneHtml(){
    const clone = document.documentElement.cloneNode(true);
    syncFormStateIntoClone(clone);
    clone.querySelectorAll('script').forEach(el => el.remove());
    clone.querySelectorAll('#upload-shell, .topbar-actions, .del-btn, .del-block, .add-act, .ghost-btn, .mini-btn').forEach(el => el.remove());
    ...
    let localCss = '';
    try{
      localCss = await fetch('./styles.css', { cache:'no-store' }).then(r => r.text());
    }catch(err){ console.warn('Could not inline local CSS', err); }

    const exportCss = `
<style>
${localCss}
body{background:#fff!important;display:block!important;min-height:auto!important;}
aside,.topbar-actions,#upload-shell,.del-btn,.del-block,.add-act,.ghost-btn,.mini-btn,.toast{display:none!important;}
.main-workspace{margin:0!important;width:100%!important;max-width:none!important;padding:24px!important;}
.topbar,.sec{box-shadow:none!important;}
.chart-export-image{display:block;max-width:100%;height:auto;}
@page{size:auto;margin:12mm;}
</style>`;

    const head = clone.querySelector('head');
    if(head) head.insertAdjacentHTML('beforeend', exportCss);
    const titleNode = clone.querySelector('title');
    if(titleNode && state.model) titleNode.textContent = `${state.model.meta.dashboardCode} BvA Dashboard`;
    return '<!DOCTYPE html>
' + clone.outerHTML;
  }
