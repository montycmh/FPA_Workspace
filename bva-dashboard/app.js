...
(function(){
  ...
  async function downloadHtml(){
    ...
    let localCss = '';
    try{
      const cssResp = await fetch('./styles.css', { cache:'no-store' });
    ...
    clone.querySelectorAll('link[rel="stylesheet"]').forEach(link => {
      ...
      if(href.endsWith('styles.css')) link.remove();
    });

    clone.querySelectorAll('.hidden, .del-btn, .del-block, .add-act, .mini-btn, .ghost-btn, .topbar-actions, #save-nav, #download-nav, #reset-btn, #build-btn, #errors, #upload-shell').forEach(el => el.remove());
    ...
    liveCanvases.forEach((canvas, idx) => {
      ...
      img.alt = target.getAttribute('aria-label') || 'chart';
      img.style.width = target.style.width || target.getAttribute('width') + 'px' || '100%';
...
button,.ghost-btn,.mini-btn,.del-btn,.del-block,.add-act{display:none!important;}
aside nav ul li{cursor:default!important;}
`;
    clone.head.appendChild(styleEl);

    const title = clone.querySelector('title');
    if(title) title.textContent = `BvA ${state.model.meta.dashboardCode} Dashboard`;

    const html = '<!DOCTYPE html>\n' + clone.outerHTML;
    const blob = new Blob([html], { type:'text/html;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `bva_dashboard_${slug(state.model.meta.dashboardCode)}_${slug(state.model.meta.fyToken)}_${slug(state.model.meta.monthToken)}.html`;
    a.click();
