...
(function(){
  ...
  let LOCAL_STYLE_TEXT = '';
  loadLocalStyles();

  async function loadLocalStyles(){
    try{
      const res = await fetch('./styles.css', { cache:'no-store' });
      if(res.ok) LOCAL_STYLE_TEXT = await res.text();
    }catch(err){
      console.warn('Could not preload styles.css for HTML export', err);
    }
  }
  ...
  function downloadHtml(){
    ...
    const localCssLink = clone.querySelector('link[href="./styles.css"], link[href$="/styles.css"], link[href$="styles.css"]');
    if(localCssLink){
      const styleTag = clone.ownerDocument.createElement('style');
      styleTag.setAttribute('data-export-inline', 'styles');
      styleTag.textContent = LOCAL_STYLE_TEXT || '';
      localCssLink.replaceWith(styleTag);
    }
    ...
    liveCanvases.forEach((canvas, idx) => {
      ...
      try{
        ...
        console.warn('Could not serialize chart canvas for export', err);
      }
    });
    ...
    clone.querySelectorAll('script[src*="xlsx"], script[src*="chart"], script[src*="Chart"]').forEach(el => el.remove());
