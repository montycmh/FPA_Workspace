...
function hcaBuildHTML({buData, months, M4, fname}){
  ...
  function dRow(label, d){
    return `<tr class="detail"><td>${label}</td><td>${d.hc[ML[0]]||0}</td><td>${d.hc[ML[1]]||0}</td><td>${d.hc[ML[2]]||0}</td><td>${mov(d.new||0,'col-new-val')}</td><td>${mov(d.att||0,'col-att-val')}</td><td>${mov(d.trans||0,'col-trans-val')}</td><td>${d.hc[ML[3]]||0}</td></tr>`;
  }
  function sRow(label, d, light){
    const cls = light ? 'subtotal-light' : 'subtotal';
    return `<tr class="${cls}"><td>${label}</td><td>${d.hc[ML[0]]||0}</td><td>${d.hc[ML[1]]||0}</td><td>${d.hc[ML[2]]||0}</td><td>${mov(d.new||0,'col-new-val')}</td><td>${mov(d.att||0,'col-att-val')}</td><td>${mov(d.trans||0,'col-trans-val')}</td><td>${d.hc[ML[3]]||0}</td></tr>`;
  }
